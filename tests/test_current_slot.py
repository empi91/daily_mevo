...
def test_current_slot_normal_cet() -> None:
    # UTC 15:00 Mon → Warsaw 16:00 Mon (CET = UTC+1, January)
    # 2026-01-05 is a Monday
    now_utc = datetime(2026, 1, 5, 15, 0, tzinfo=UTC)
    now_warsaw = now_utc.astimezone(WARSAW)

    day_of_week, time_slot = _current_slot(now=now_warsaw)

    assert day_of_week == 0  # Monday
    assert time_slot == time(16, 0)
