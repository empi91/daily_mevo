...
| B-01 | auth-session-fix          | Auth session persists after login on production domain          | S-02          | FR-008, FR-009                                    | ready    |
...
- **Unknowns:**
  - Root cause not yet confirmed — candidates: `cookie_domain=None` with `Secure=True`, Mikr.us reverse proxy stripping `Set-Cookie`, HTTPS redirect eating the cookie, or `SameSite=Lax` blocking cross-origin requests. See `context/changes/auth-session-fix/change.md` for the full candidate list and diagnostic steps.
- **Risk:** Cookie behavior differs between localhost and production (HTTPS, different domain); fix must be verified in the actual production environment, not just integration tests.
- **Acceptance test:** `E2E_BASE_URL=https://dailymevo.pl npx playwright test e2e/auth-session.spec.ts` — all 3 tests must pass against production. Cookie attribute test (`context.cookies()`) reveals the exact browser-received attributes for diagnosis.
- **GitHub issue:** #24
- **Status:** ready
...
| B-01       | auth-session-fix          | Auth session not persisting after login on production | yes                   | Blocks S-03; cookie/CORS investigation needed   |
| B-02       | db-storage-fix            | Supabase DB exceeding 0.5GB free plan limit           | —                     | ✅ Implemented 2026-06-19                       |
| S-03       | favourites-dashboard      | Favourites and personal dashboard                    | yes                   | Depends on S-01 (done), S-02 (done), B-01      |
