...
MevoStats — a web app that collects Mevo bike-sharing station snapshots (Tricity: Gdańsk/Gdynia/Sopot) every ~5 minutes via the public Mevo Open Data API and presents historical availability patterns (average bikes per 15-min slot per day-of-week) so commuters can plan ahead.

## Tech Stack

- **Backend**: FastAPI (Python 3.12), async, fully typed with Pydantic
- **Database**: PostgreSQL (Supabase preferred), async access via asyncpg
- **Frontend**: TBD (React/Vite planned per tech-stack docs)
- **Background jobs**: APScheduler or system cron for 5-min data collection
- **Auth**: fastapi-users or lightweight JWT
- **Package manager**: uv
- **Linting/formatting**: ruff
- **Type checking**: mypy
- **Deployment target**: Fly.io (placeholder, final decision pending)
...
```bash
# Start a test-only Postgres (one-time setup)
docker run -d --name mevo-test-db -p 5433:5432 -e POSTGRES_PASSWORD=test postgres:16

# Set required env vars for test collection
...
Integration tests are marked with `@pytest.mark.integration` and require `MEVO_TEST_DATABASE_URL` to be set. They skip gracefully when the env var is absent.

## Architecture (planned)

The project is freshly scaffolded — `main.py` is a stub. Per the PRD (`context/foundation/prd.md`), the core components are:

1. **Data collector** — scheduled task that polls the Mevo API, stores raw snapshots in PostgreSQL
2. **Aggregation layer** — computes average availability per station per 15-min timeslot per day-of-week
3. **Station API** — FastAPI endpoints serving station data, charts, and search
4. **Auth module** — registration, login, favourites (registered users only)
5. **Frontend** — station pages with availability charts, personal dashboard for favourites

## Key Context Files

- `context/foundation/prd.md` — full product requirements
- `context/foundation/tech-stack.md` / `tech-stack-v2.md` — stack decisions and rationale
- `context/foundation/shape-notes.md` — original shaping notes
